...
- **Hunter wake-up**: When a Hunter dies, confirm every player sees the public wake-up event before the Hunter chooses a target.
...
- **Witch same-night actions**: Use heal and poison in one night in quick succession; both potions must be consumed and both effects must resolve.
...
- **Win condition - one versus one**: With one Werewolf and one remaining village player, verify the game ends immediately with a Werewolf win.
- **Win condition - final Hunter shot**: If a dead Hunter shoots the last Werewolf and no other player remains, verify the Werewolf team wins the simultaneous final death.
...
- **Early vote with abstentions**: If three of five living players abstain (or have not voted when the host ends voting) and two vote for one target, verify nobody is eliminated.
- **Room invite link**: In the lobby, share/copy the room link and open it in a fresh browser; verify the join form is prefilled with the room code.
...
- **Player leaves mid-day-vote**: With a living player leaving during the day vote, verify the vote is re-evaluated with the remaining players (not silently skipped): the day is only marked resolved when the remaining players have voted or a strict majority of abstentions/missing votes exists.
