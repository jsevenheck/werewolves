...
class Narrator {
  ...
  private stop() {
    ...
    for (const [key, howl] of this.howls.entries()) {
  ...
  private getHowl(key: string) {
    ...
    if (this.howls.size > MAX_CACHED_HOWLS) {
      const [oldestKey, oldestHowl] = this.howls.entries().next().value;
      oldestHowl.unload();
      this.howls.delete(oldestKey);
    }
    return howl;
  }
}
